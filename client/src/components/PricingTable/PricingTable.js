      ...
      <Grid container spacing={2} justifyContent="center">
        {services.length > 0 ? (
          services.map((service, index) => (
            <Grid item xs={12} sm={6} md={3} lg={2} key={index}>  {/* Giảm kích thước ở đây */}
              <Card
                sx={{
                  display: "flex",
              ...
              >
                <CardContent sx={{ display: "flex", flexDirection: "column", height: "100%" }}>
                  <Typography variant="h6" fontWeight="bold" sx={{ flexShrink: 0, fontSize: "16px" }}>
                    {service?.name}
                  </Typography>
                  <Typography variant="h6" sx={{ color: "primary.main", mt: 1, fontSize: "14px" }}>
                    {service?.price} VND
                  </Typography>
                  <Typography variant="body2" color="text.secondary" sx={{ flexGrow: 1, fontSize: "13px" }}>
                    {service?.description}
                  </Typography>
                </CardContent>
              </Card>
